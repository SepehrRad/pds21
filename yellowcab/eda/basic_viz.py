...
def basic_plots(df, borough=None):
    ...
    if borough is not None:
        ...
        df = df.loc[
            (df["PULocationID"].isin(borough_loc_ids))
            | (df["DOLocationID"].isin(borough_loc_ids))
        ]

    df_agg_count_monthly = yellowcab.eda.agg_stats(
        df["pickup_datetime"].dt.month, df["pickup_month"], ["count"]
    )
    df_agg_mean_duration_monthly = yellowcab.eda.agg_stats(
        df["pickup_datetime"].dt.month, df["trip_duration_minutes"], ["mean"]
    )
    df_agg_mean_passenger_mothly = yellowcab.eda.agg_stats(
        df["pickup_datetime"].dt.month, df["passenger_count"], ["mean"]
    )

    df_agg_count_weekly = yellowcab.eda.agg_stats(
        df["pickup_datetime"].dt.week, df["pickup_month"], ["count"]
    )
    df_agg_mean_duration_weekly = yellowcab.eda.agg_stats(
        df["pickup_datetime"].dt.week, df["trip_duration_minutes"], ["mean"]
    )
    df_agg_mean_passenger_weekly = yellowcab.eda.agg_stats(
        df["pickup_datetime"].dt.week, df["passenger_count"], ["mean"]
    )

    fig, axes = plt.subplots(2, 3, figsize=(15, 10))

    axes[0, 0].bar(
        df_agg_count_monthly.index, df_agg_count_monthly["count_pickup_month"]
    )
    axes[0, 0].set_xlabel("Month")
    axes[0, 0].set_ylabel("Number of trips")

    axes[0, 1].bar(
        df_agg_mean_duration_monthly.index,
        df_agg_mean_duration_monthly["mean_trip_duration_minutes"],
    )
    axes[0, 1].set_xlabel("Month")
    axes[0, 1].set_ylabel("Avg trip duration")

    axes[0, 2].bar(
        df_agg_mean_passenger_mothly.index,
        df_agg_mean_passenger_mothly["mean_passenger_count"],
    )
    ...
    axes[1, 1].bar(
        df_agg_mean_duration_weekly.index,
        df_agg_mean_duration_weekly["mean_trip_duration_minutes"],
    )
    axes[1, 1].set_xlabel("Week")
    axes[1, 1].set_ylabel("Avg trip duration")

    axes[1, 2].bar(
        df_agg_mean_passenger_weekly.index,
        df_agg_mean_passenger_weekly["mean_passenger_count"],
    )
...
def airport_plots(df):
    ...
    """
    df_airport_trips = df.loc[
        (df["PULocationID"].isin(["1", "132", "138"]))
        | (df["DOLocationID"].isin(["1", "132", "138"]))
    ]

    df_agg_count_airport = yellowcab.eda.agg_stats(
        df_airport_trips["pickup_datetime"].dt.week,
        df_airport_trips["pickup_month"],
        ["count"],
    )
    df_agg_mean_duration_airport = yellowcab.eda.agg_stats(
        df_airport_trips["pickup_datetime"].dt.week,
        df_airport_trips["trip_duration_minutes"],
        ["mean"],
    )
    df_agg_mean_passenger_airport = yellowcab.eda.agg_stats(
        df_airport_trips["pickup_datetime"].dt.week,
        df_airport_trips["passenger_count"],
        ["mean"],
    )
    ...
    axes[1].bar(
        df_agg_mean_duration_airport.index,
        df_agg_mean_duration_airport["mean_trip_duration_minutes"],
    )
    axes[1].set_xlabel("Week")
    axes[1].set_ylabel("Mean trip duration")

    axes[2].bar(
        df_agg_mean_passenger_airport.index,
        df_agg_mean_passenger_airport["mean_passenger_count"],
    )
