...
def _load_models(manhattan: bool = False):
    ...
    if manhattan:
        fare_amount_model = pickle.load(
            open(f"{model_path}/xgb_model_fare_amount_optimized_manhattan.pkl", "rb")
        )
        trip_distance_model = pickle.load(
            open(f"{model_path}/xgb_model_trip_distance_optimized_manhattan.pkl", "rb")
        )
        payment_type_model = pickle.load(
            open(f"{model_path}/xgb_model_payment_type_optimized_manhattan.pkl", "rb")
        )
    else:
        fare_amount_model = pickle.load(
            open(f"{model_path}/xgb_model_fare_amount_optimized.pkl", "rb")
        )
        trip_distance_model = pickle.load(
            open(f"{model_path}/xgb_model_trip_distance_optimized.pkl", "rb")
        )
        payment_type_model = pickle.load(
            open(f"{model_path}/xgb_model_payment_type_optimized.pkl", "rb")
        )
...
def transform(
    ...
    output_file_path: str = input_output.get_data_path(),
    verbose=False,
):
    """
    ...
    input_output.write_parquet_file(
        df=df_cleaned, filename=output_file_name, path=output_file_path
    )
...
def predict(
    ...
    fare_amount_model, trip_distance_model, payment_type_model = _load_models(
        manhattan=manhattan
    )
    (
        fare_amount_relevant_features,
    ...
    _ = _make_data_preparation(
        df=df_augmented,
        relevant_features=fare_amount_relevant_features,
    ...
    _ = _make_data_preparation(
        df=df_augmented,
        relevant_features=trip_distance_relevant_features,
        is_manhattan=manhattan,
    )
    ...
    _ = _make_data_preparation(
        df=df_augmented,
        relevant_features=payment_type_relevant_features,
    ...
    input_output.write_parquet_file(
        df=df, filename=output_file_name, path=output_file_path
    )
