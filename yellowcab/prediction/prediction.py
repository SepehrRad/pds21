...
def _make_data_preparation(df, prediction_type, target):
    ...
    if prediction_type == "regression":
        if target == "trip_distance":
            ...
            new_df.drop(
                column_description.get("temporal_features"), inplace=True, axis=1
            )
            new_df.drop(
                column_description.get("spatial_features"), inplace=True, axis=1
            )
        ...
        if target == "fare_amount":
            ...
            cols
            df = df[cols]
            column_description["categorical_features"] = df.loc[
                :, df.columns.str.contains("^Zone")
            ].columns.tolist()
        ...
        column_description["categorical_features"] = [
            "RatecodeID",
            "Season",
        ]  # payment type should not be transofrmed here
        new_df = transform_columns(df, column_description)
...
def _print_prediction_scores(prediction_type, y_test, X_test, pipeline):
    ...
    if prediction_type == "classification":
        ...
        print(
            "RMSE: ",
            np.sqrt(metrics.mean_squared_error(y_test, pipeline.predict(X_test))),
        )
...
def make_baseline_predictions(df):
    ...
    make_predictions(
        ...
        model=regression_model,
        scaler_type="standard_scaler",
        model_name="base_reg_fareamount",
    )
